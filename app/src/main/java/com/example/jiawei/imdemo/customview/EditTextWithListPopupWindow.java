...
public class EditTextWithListPopupWindow extends EditText {
    ...
    private void init(Context context) {
        ...
        if (rightDrawable == null) {
            rightDrawable = getResources().getDrawable(R.drawable.tencent_tls_ui_down_arrow);
        }
        rightDrawable.setBounds(0, 0, rightDrawable.getIntrinsicWidth(), rightDrawable.getIntrinsicHeight());
        setCompoundDrawables(getCompoundDrawables()[0],
                getCompoundDrawables()[1], rightDrawable, getCompoundDrawables()[3]);

        list = getResources().getStringArray(R.array.tencent_tls_ui_countryCode);

        listPopupWindow = new ListPopupWindow(context);
        listPopupWindow.setAdapter(new ArrayAdapter<String>(context,
                R.layout.tencent_tls_ui_item, list));
    ...
    private void setArrow(int flag) {
        Drawable drawable;
        if (UP_ARROW == flag) {
            drawable = getResources().getDrawable(R.drawable.tencent_tls_ui_up_arrow);
        } else {
            drawable = getResources().getDrawable(R.drawable.tencent_tls_ui_down_arrow);
        }
